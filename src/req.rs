...
pub async fn post (client: &reqwest::Client, full_url: &String, data: String) -> String {
    let request = client.post(full_url).header("Content-Type", "application/json").body(data).build().unwrap();
...
async fn process_response (response: reqwest::Response) -> String {
    ...
    panic!("Server error: status code: {}, response text: {}", status_code, text); 
}