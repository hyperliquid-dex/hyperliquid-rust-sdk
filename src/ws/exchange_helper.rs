...
use crate::{
    Error, BulkOrder, 
    exchange::{BuilderInfo, order::OrderRequest}, 
    helpers::next_nonce, 
    prelude::*, 
...
pub(crate) async fn bulk_order_with_builder(
    ...
    let wallet = wallet.as_ref().ok_or(Error::JsonParse("Wallet not provided".to_string()))?;
    ...
    let action_value = serde_json::to_value(&action).map_err(|e| Error::JsonParse(e.to_string()))?;
...
mod tests {
    ...
    use crate::{Order, exchange::order::Limit};
    ...
    async fn test_send_order() {
        ...
        let payload = bulk_order_with_builder(
                vec![order],
                Some(&wallet),
                builder,
                None,
        ...
        match result {
            Ok(response) => {
                ...
                if response_content.payload.status != "ok" {
                    eprintln!("Request failed with status: {}", response_content.payload.status);
                    return;
                }
                ...
                if let Some(data_content) = &response_content.payload.response {
                    if let Some(statuses) = data_content.data.get("statuses") {
                                ...
                                for (i, status) in order_statuses.into_iter().enumerate() {
                                    match status {
                                        OrderStatus::Filled { filled } => {
                                            println!("✓ Order {} was filled with OID: {}", i, filled.oid);
                                            if let (Some(sz), Some(px)) = (&filled.total_sz, &filled.avg_px) {
                                                println!("   Size: {}, Avg Price: {}", sz, px);
                                            }
                                        }
                                        OrderStatus::Resting { resting } => {
                                            println!("✓ Order {} is resting with OID: {}", i, resting.oid);
                                        }
                        ...
                        println!("No statuses found in response data: {:#?}", data_content.data);
                    }
                } else {
