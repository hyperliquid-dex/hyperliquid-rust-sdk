...
impl WsManager {
    ...
    pub(crate) async fn new(url: String, reconnect: bool) -> Result<WsManager> {
        ...
        let pending_responses: Arc<Mutex<HashMap<u64, tokio::sync::oneshot::Sender<serde_json::Value>>>> = 
            ...
            let reader_fut = async move {
                while !stop_flag.load(Ordering::Relaxed) {
                    if let Some(data) = reader.next().await {
                        if let Err(err) =
                            WsManager::parse_and_send_data(data, &subscriptions_copy, &pending_responses_copy).await
                        {
    ...
    fn get_identifier(message: &Message) -> Result<String> {
        match message {
            ...
            .map_err(|e| Error::JsonParse(e.to_string())),
            Message::SubscriptionResponse | Message::Pong | Message::Error(_) | Message::Post(_) => Ok(String::default()),
            Message::SubscriptionResponse | Message::Pong | Message::Error(_) | Message::Post(_) => Ok(String::default()),
   ...
   async fn parse_and_send_data(
        ...
        Ok(data) => match data.into_text() {
            Ok(data) => {
                ...
                match serde_json::from_str::<serde_json::Value>(&data) {
                    Ok(json_value) => {
                        match WsResponse::try_from(json_value.clone()) {
                            Ok(response) => {
                                match response {
    ...
    pub(crate) async fn post(
        ...
        nonce: u64,
            ) -> Result<WsPostResponse> {  // Changed return type
                ...
                match tokio::time::timeout(Duration::from_secs(10), rx).await {
                    Ok(Ok(response)) => {
                        ...
                        let ws_response: WsPostResponse = serde_json::from_value(response)
                            .map_err(|e| Error::JsonParse(format!("Failed to parse WsPostResponse: {}", e)))?;
                        Ok(ws_response)
                    }
                    Ok(Err(_)) => {
                    ...
                    Err(_) => {
                        ...
                        Err(Error::JsonParse("Request timed out after 10 seconds".to_string()))
                    }
                }
            }
    }
