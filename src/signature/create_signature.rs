use crate::{prelude::*, signature::agent::l1, signer::Signer, Error};
use ethers::types::{transaction::eip712::Eip712, Signature, H256};
...
pub(crate) async fn sign_typed_data<T: Eip712>(
    payload: &T,
    wallet: &dyn Signer,
) -> Result<Signature> {
    let encoded = payload
    ...
    wallet.secp256k1_sign(H256::from(encoded)).await
}
...
mod tests {
    ...
    fn get_wallet() -> Result<Arc<dyn Signer>> {
    ...
    async fn test_sign_l1_action() -> Result<()> {
        ...
        assert_eq!(
            sign_l1_action(wallet.as_ref(), connection_id, true)
                .await?
                .to_string(),
        ...
        assert_eq!(
            sign_l1_action(wallet.as_ref(), connection_id, false)
                .await?
                .to_string(),
    ...
    async fn test_sign_usd_transfer_action() -> Result<()> {
        ...
        assert_eq!(
            sign_typed_data(&usd_send, wallet.as_ref())
                .await?
                .to_string(),
            expected_sig
        );
